...
export async function getAllActiveCategories(): Promise<CategoryRow[]> {
  // productCount counts the category itself AND its descendants (depth ≤2 tree),
  // so a depth-0 parent like "IT & Gadget" shows the total of products in all its
  // grandchildren, not 0 (since Apify products live in depth-2 leaves).
             ...
             WHERE p.is_active = true
               ...
               AND p.category_id IN (
                 SELECT cc.id FROM categories cc
                  WHERE cc.id = c.id
                     OR cc.parent_id = c.id
                     OR cc.parent_id IN (SELECT id FROM categories WHERE parent_id = c.id)
               )
           ) AS "productCount"
      FROM categories c
...
export async function getTopProductsInCategory(
  ...
  // Returns products in this category OR any of its descendants (depth ≤2 tree).
  // This makes depth-0 "IT & Gadget" show all products from its leaf grandchildren.
       ...
       AND p.category_id IN (
         SELECT cc.id FROM categories cc
          WHERE cc.id = ${categoryId}
             OR cc.parent_id = ${categoryId}
             OR cc.parent_id IN (SELECT id FROM categories WHERE parent_id = ${categoryId})
       )
